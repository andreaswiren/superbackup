...
/// Render Markdown well enough to read a README on a panel.
///
/// Not a Markdown implementation — a *reading* one. Headings get their weight,
/// list items get a bullet and a hang, fenced code keeps its monospace and its
/// line breaks, and inline `code`, **bold** and _italic_ stop showing their
/// punctuation. Everything else is left as written, because a document that
/// renders imperfectly is still readable and one that renders as a wall of
/// asterisks is not.
///
/// Deliberately no images, no HTML, and no link navigation: this displays a
/// file out of a repository that may not be the user's, and the safe thing for
/// such a file to be able to do is *nothing* except be read. A link's text and
/// its target are both shown so the reader can see where it would go.
pub fn markdown(ui: &mut Ui, source: &str, width: f32) {
    let t = theme::tokens(ui.ctx());
    ...
    for raw in source.lines() {
        ...
        if in_code {
            text(ui, line, Type::MonoSmall, t.text_secondary);
            continue;
        }
        ...
        // Headings. `#` through `###`; deeper ones read as body text, which is
        // what they look like in most rendered Markdown anyway.
        let hashes = trimmed.chars().take_while(|c| *c == '#').count();
        if hashes > 0 && trimmed.chars().nth(hashes) == Some(' ') {
            let body = strip_inline(&trimmed[hashes + 1..]);
            let ty = match hashes {
                1 => Type::H1,
                2 => Type::H2,
                _ => Type::H3,
            ...
            ui.add_space(space::M);
            paragraph_at(ui, body, ty, t.text_primary, width);
            ui.add_space(space::XS);
            continue;
        }
        ...
        // List items, ordered and not. The indent is preserved so nested
        // lists still read as nested.
        ...
        if let Some(item) = bullet {
            ui.horizontal_top(|ui| {
                ui.add_space(indent as f32 * 4.0 + space::M);
                text(ui, "•", Type::Body, t.text_muted);
                ui.add_space(space::S);
                paragraph_at(ui, strip_inline(item), Type::Body, t.text_secondary, width - 40.0);
            });
            continue;
        }
        ...
        if let Some(quote) = trimmed.strip_prefix("> ") {
            ui.horizontal_top(|ui| {
                ui.add_space(space::M);
                vertical_rule(ui, 16.0);
                ui.add_space(space::S);
                paragraph_at(ui, strip_inline(quote), Type::Body, t.text_muted, width - 40.0);
            });
            continue;
        }

        paragraph_at(ui, strip_inline(trimmed), Type::Body, t.text_secondary, width);
    }
}

/// Take the punctuation out of inline markup, leaving the words.
///
/// egui has no rich text run inside a paragraph here, so emphasis cannot be
/// *shown* — but showing `**important**` with its asterisks is worse than
/// showing `important`, and a link renders as `text (url)` so the destination
/// is visible rather than hidden behind words.
fn strip_inline(source: &str) -> String {
    ...
    while i < chars.len() {
        match chars[i] {
            ...
            // Emphasis and inline code markers, dropped.
            '*' | '_' | '`' => {
                let marker = chars[i];
...
/// Parse `[text](url)` starting at `open`, returning the text, the url, and
