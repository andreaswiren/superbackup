...
impl App {
    ...
    fn destination_card(
        ...
        widgets::card_tinted(ui, None, failed.then(|| theme::alpha(t.danger.mark, 0.4)), |ui| {
            ui.set_width(ui.available_width());
            ui.horizontal(|ui| {
                let icon = self
                    .data
            ...
            match &destination.snapshot_id {
                Some(id) => {
                    ...
                    widgets::kv_with(ui, copy::run::DETAIL_SNAPSHOT, |ui| {
                        ...
                        if widgets::icon_button_compact(ui, Icon::Copy, copy::action::COPY, true)
                            .clicked()
                        {
                ...
                None => {
                    widgets::kv(
                        ui,
                        copy::run::DETAIL_SNAPSHOT,
                        copy::run::DETAIL_NO_SNAPSHOT,
                        false,
                    );
                }
            }
            ...
            if !destination.warnings.is_empty() {
                ...
                if self.screens.run_detail.expanded_warnings == Some(destination.destination_id) {
                    ui.add_space(space::M);
                    widgets::code_block(
                        ui,
                ...
                widgets::card_tinted(
                    ...
                    |ui| {
                        ...
                        if let Some(hint) = &error.hint {
                            ui.add_space(space::L);
                            widgets::banner(ui, widgets::BannerKind::Info, hint, None, |_| {});
                        }
                        if let Some(detail) = &error.detail {
                            ...
                            if widgets::link(ui, label).clicked() {
                                self.screens.run_detail.expanded_details =
                                    if expanded { None } else { Some(destination.destination_id) };
                            }
                            if expanded {
                                ui.add_space(space::M);
