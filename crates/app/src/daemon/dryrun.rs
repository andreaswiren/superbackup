...
use superbackup_core::engine::{CancelToken, EngineEvent, EVENT_CHANNEL_CAPACITY};
...
/// The engine event a dry run must never be mistaken for.
///
/// Exposed so the event pump can annotate a dry run's activity-log line; the
/// runner itself has no notion of a rehearsal, so the distinction has to be
/// made by whoever started it.
pub fn is_dry_run_event(event: &EngineEvent) -> bool {
    matches!(event, EngineEvent::RunFinished { run } if run
        .destinations
        .iter()
        .any(|d| d.warnings.iter().any(|w| w.starts_with("Dry run:"))))
}
