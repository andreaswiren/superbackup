...
///
/// It is deliberately environment-independent: the driver clears the child
/// environment, so the fake reads its instructions from a `control.txt` sitting
/// beside its own executable, and writes its recording to a `record.txt` in the
/// same directory. Copying the executable into a per-test directory therefore
/// gives every test an isolated, independently scripted kopia.
const FAKE_KOPIA_SRC: &str = r##"
...
fn fake_kopia_template() -> Result<&'static Path, &'static str> {
    ...
    let built = BUILT.get_or_init(|| {
        ...
        let exe = dir.join(exe_name("kopia-template"));
...
impl Scenario {
    fn new(name: &str) -> Scenario {
        let template = match fake_kopia_template() {
            Ok(p) => p.to_path_buf(),
            Err(_) => PathBuf::new(),
        };
    ...
    fn script(&self, entries: &[(&str, &str)]) {
        let body: String =
            entries.iter().map(|(k, v)| format!("{k}={v}\n")).collect::<Vec<_>>().join("");
        std::fs::write(self.bin_dir.join("control.txt"), body).expect("write control");
    }

    /// Point the fake at a file to print on stdout.
    fn stdout_file(&self, name: &str, contents: &str) -> PathBuf {
    ...
    fn binary(&self) -> KopiaBinary {
        KopiaBinary::assume(&self.exe, KopiaVersion::new(0, 21, 5), KopiaSource::Configured)
    }

    fn record(&self) -> Vec<Invocation> {
        let text = std::fs::read_to_string(self.bin_dir.join("record.txt")).unwrap_or_default();
        parse_record(&text)
    }
...
impl Invocation {
    ...
    fn flag_values(&self, flag: &str) -> Vec<String> {
        let prefix = format!("{flag}=");
...
fn parse_record(text: &str) -> Vec<Invocation> {
...
#[tokio::test]
async fn prefers_the_bundled_build_over_one_on_path() {
    let template = fake_or_skip!();
    let s = Scenario::new("bundled");
    let paths = s.paths();

    // Install the fake where `paths.bundled_kopia()` looks for it.
    let bundled = paths.bundled_kopia();
    std::fs::create_dir_all(bundled.parent().expect("bin parent")).expect("bin dir");
    std::fs::copy(template, &bundled).expect("install bundled kopia");
    std::fs::write(
        bundled.parent().expect("bin parent").join("control.txt"),
        "version=0.20.1 build: bundled from: kopia/kopia\n",
    )
    .expect("control");

    let bin = KopiaBinary::discover(&Settings::default(), &paths).await.expect("discovers");
    assert_eq!(bin.source(), KopiaSource::Bundled);
    assert_eq!(bin.version(), &KopiaVersion::new(0, 20, 1));
    assert_eq!(bin.path(), bundled.as_path());
}

#[tokio::test]
async fn falls_back_to_the_kopia_on_path() {
    let _ = fake_or_skip!();
    let s = Scenario::new("onpath");
    s.script(&[("version", "0.19.0 build: onpath from: kopia/kopia")]);

    // `which` reads PATH from the environment, so this is the only way to
    // exercise the third discovery branch. Restored immediately afterwards.
    let original = std::env::var_os("PATH");
    let mut entries = vec![s.bin_dir.clone()];
    if let Some(p) = &original {
        entries.extend(std::env::split_paths(p));
    }
    let joined = std::env::join_paths(entries).expect("join PATH");
    // Safety: single-threaded within this test, and PATH is restored below.
    std::env::set_var("PATH", &joined);

    let discovered = KopiaBinary::discover(&Settings::default(), &s.paths()).await;
    ...
    let bin = discovered.expect("discovers on PATH");
